...
router.get("/get/report/write", async (req, res) => {
  try {
    ...
    const query = `SELECT small_goal,mini1,mini2,mini3,mini4,cmini1,cmini2,cmini3, cmini4,total,date
  ...
  try {
    ...
    try {
      ...
      for (const result of results) {
        ...
        jarray.push({
          datetime: result[0].date,
          main: mains[i],
          main_id: result[0].main_id,
          small1: result[0].small_goal,
          small2: result[1].small_goal,
          small3: result[2].small_goal,
          small4: result[3].small_goal,
          total: score,
        });
        // jarray.push(result);
        i = i + 1;
...
router.post("/report/demo", async (req, res) => {
  try {
    ...
    var bucketarray = [
      bucketlist.MainKeyword1,
      bucketlist.MainKeyword2,
      bucketlist.MainKeyword3,
      bucketlist.MainKeyword4,
    ];
              ...
              if (!err) {
                ...
                for (i = 0; i < bucketarray.length; i++) {
                  const query = `INSERT INTO big_goal(user_email, main_id, small_goal, mini1, mini2, mini3, mini4) 
                        values ('${user_email}','${main_id}','${bucketarray[i].Value}','${bucketarray[i].Detail1}','${bucketarray[i].Detail2}','${bucketarray[i].Detail3}','${bucketarray[i].Detail4}');`;
                  sendarray.push({
                    small_goal: bucketarray[i].Value,
                    mini: [
                      bucketarray[i].Detail1,
                      bucketarray[i].Detail2,
                      bucketarray[i].Detail3,
                      bucketarray[i].Detail4,
                    ],
                  });
                  maria.query(query, function (err, rows, fields) {
                    if (err) {
